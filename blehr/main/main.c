...
#define MAX_INCREMENT_VALUE 0x7FFFFF
#define INCREMENT_INTERVAL_US 10000000 // 10 seconds in microseconds
...
blehr_tx_hrate(TimerHandle_t ev)
{
    ...
    uint32_t intValue = (hrm[0] << 16) | (hrm[1] << 8) | hrm[2];
    intValue++;
        vTaskDelay(10000/portTICK_PERIOD_MS);


    if (intValue > MAX_INCREMENT_VALUE) {
        intValue = 0; // Reset value if overflow occurs
    }

    hrm[0] = (intValue >> 16) & 0xFF; /* contact of a sensor */
    hrm[1] = (intValue >> 8) & 0xFF;
    hrm[2] =  intValue & 0xFF;
